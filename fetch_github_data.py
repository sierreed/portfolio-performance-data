...
# === Step 2: Load Tickers from Excel ===
...
try:
    ...
    # Save as CSV (better for GitHub & GPT)
    csv_path = os.path.expanduser("~/Documents/stock_data.csv")
    df_stock_data.to_csv(csv_path, index=False)

    print(f"✅ Successfully updated stock data for {len(tickers)} stocks and saved to {csv_path}")
...
try:
    ...
    subprocess.run(["git", "commit", "-m", "Auto-update stock data"], check=True)
